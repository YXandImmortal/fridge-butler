...
## [release 0.5.0] - 2026-06-24

> release 0.5.0 为重要版本更新，在第一部分游戏化系统基础上，补齐采购助手、物品智能推荐、批量操作及 AI 交互能力，并新增采购方案邮件发送等收尾功能，标志着 0.5.0 主体功能基本完成。
...
#### 后端
- **采购助手核心模块**：
  - 采购方案全生命周期管理：支持手动创建、模板创建、AI 推荐生成三种来源。
  - 方案状态流转：待采购、已取消、已入库等状态及状态校验。
  - 采购任务视图：按待采购方案展示物品清单与进度。
  - 核对入库结算：支持逐项核对计划数量与实际数量，自动完成入库并生成物品记录。
- **AI 采购推荐服务**：
  - 日常采购推荐：基于冰箱现有食材与消耗习惯生成采购清单。
  - 特殊场景推荐：支持 `baby-food`、`bbq`、`family-gathering`、`hotpot` 等场景生成主题采购方案。
  - 存储位置建议：为推荐物品补充默认存放位置。
  - 数据充足度判断：在库存数据不足时给出友好提示，避免生成低质量推荐。
- **用户采购计划模板**：支持创建个人模板、从已有方案保存为模板、使用模板快速创建新方案。
- **物品智能推荐服务**：根据物品名称 AI 推荐分类、单位、存储位置、存放日期等字段，降低用户录入成本。
- **物品批量取出**：支持一次性取出多件物品，自动记录取出日志并触发游戏化结算。
- **数据快照增强**：物品、添加记录、取出记录增加存储位置与单位快照字段，便于历史追溯。

#### 前端
- **采购助手三视图**：新增「制定计划」、「采购任务」、「核对入库」三个页面及相关组件 `PurchasePlanView`、`PurchaseTaskView`、`PurchaseCheckInView`。
- **批量取出对话框**：在物品管理页新增批量取出交互，支持勾选多物、统一填写取出数量与日期。
- **物品字段 AI 推荐交互**：物品创建/编辑弹窗中，输入名称后自动调用 AI 推荐并一键填充分类、单位、位置等字段。
- **扩展基础 UI 组件**：`InputDialog` 支持更丰富的字段配置与校验；`CustomSelect` 新增尺寸与变体属性，适配更多表单场景。

#### 集成与配置
- 数据库脚本 `fridge-butler-database.sql` 同步更新：新增采购方案/明细/模板相关表，扩展物品与记录表字段。
- `SecurityConfig` 开放/适配采购助手、AI 推荐、模板等新增接口权限。
- 游戏化系统与采购、物品推荐、批量取出等新场景打通，经验获取与徽章触发覆盖更完整。

### 优化
- **AI 聊天提示词全面调优**：意图识别、通用对话、菜谱推荐、每日贴士、容量估算等提示词补充上下文处理与示例，提升识别准确率与回答稳定性。
- **游戏化系统联动完善**：补充 `ResultCode` 与业务异常处理，确保游戏化结算失败时不影响主业务流程。
- **验证码工具类**：新增 `EmailUtil` 邮箱格式校验工具，统一前后端邮箱格式校验逻辑。

### 新增与优化（0.5.0 正式发布收尾）
- **版本号统一升级**：后端 `pom.xml`、前端 `package.json`、后端 `application.yaml` 同步升级至 `release 0.5.0` / `0.5.0`，构建时间更新为 `20260624-04`。
- **采购方案邮件发送**：
  - 后端新增 `POST /purchase/plan/{id}/send-email` 接口，将待采购方案以 HTML 邮件形式发送到当前用户绑定邮箱。
  - 后端引入 `spring-boot-starter-thymeleaf` 作为邮件模板引擎，新增 `templates/mail/purchase-plan.html` 采购方案邮件模板。
  - 后端新增 `GET /user/email/bound` 接口，供前端判断用户是否已绑定合法邮箱。
  - 前端采购任务页新增「发送至邮箱」按钮，未绑定邮箱时引导用户先完成绑定。
- **游戏化细节调优**：
  - `StreakDailyCheckJob` 每日冰鲜判定指定 `Asia/Shanghai` 时区，避免服务器时区不一致导致判定异常。
  - 连续冰鲜天数加成 EXP 计算由 `streak / 7`  capped at 20 调整为 `streak * 3` 且绕过每日上限，鼓励用户保持连续。
  - `BadgeCode.BATCH_MASTER` 徽章名称由「批量达人」调整为「采购达人」，与采购助手主题更贴合。
- **文档更新**：
  - `SystemController.java` 系统更新日志新增面向普通用户的 `release 0.5.0` 重大更新记录。
  - `README.md` 更新版本徽章、核心功能描述、开发路线图与界面预览说明。

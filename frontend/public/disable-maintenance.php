 ...
 *
 * 使用方法：
 *   浏览器访问：https://你的域名/disable-maintenance.php?token=你设置的密码
 */
...
if (file_exists($maintenanceFile)) {
    ...
    $duration = time() - (int)$startedAt;

    unlink($maintenanceFile);

    echo "✅ 维护模式已关闭<br>";
    echo "⏰ 开启时间：{$startedTime}<br>";
    echo "⏳ 持续时长：" . humanDuration($duration) . "<br>";
    echo "🚀 网站已恢复正常访问<br>";
} else {
    echo "ℹ️ 维护模式当前未开启，无需关闭<br>";
    echo "🚀 网站处于正常运行状态<br>";
}

echo "<br>📝 当前访问 IP：{$clientIp}<br>";
echo "💡 提示：访问 <a href=\"enable-maintenance.php?token=" . htmlspecialchars($accessToken, ENT_QUOTES, 'UTF-8') . "\">enable-maintenance.php</a> 可重新开启维护模式<br>";

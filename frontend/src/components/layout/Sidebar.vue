<template>
  <aside class="app-sidebar">
    ...
    <!-- 每日小贴士 -->
    <div v-if="userStore.roleId !== 1" class="daily-tip-section">
...
import {getTodayTip} from '@/api/dailyTip.js'
...
// 管理员侧边栏菜单
const adminFeatures = [
  {id: 'admin-dashboard', name: '数据看板', icon: 'icon-chart', path: '/super-admin/dashboard'},
  {id: 'admin-users', name: '用户管理', icon: 'icon-users', path: '/super-admin/users'},
  {id: 'admin-activation-keys', name: '密钥管理', icon: 'icon-key', path: '/super-admin/activation-keys'},
  {id: 'admin-logs', name: '操作日志', icon: 'icon-script-text', path: '/super-admin/logs'},
  {id: 'admin-notification', name: '通知发布', icon: 'icon-megaphone', path: '/super-admin/notification'},
  {id: 'admin-system', name: '系统配置', icon: 'icon-sliders', path: '/super-admin/system'},
]

// 根据角色切换菜单
const menuFeatures = computed(() => {
  if (userStore.roleId === 1) {
    return adminFeatures
  }
  return userIndexFeatures
})
...
const autoExpandActiveParent = () => {
  menuFeatures.value.forEach(feature => {
    if (hasChildren(feature)) {

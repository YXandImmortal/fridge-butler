...
const buttonClasses = computed(() => {
  ...
  if (props.type === 'link' && props.color !== 'default') {
    classes.push(`custom-button--link-${props.color}`)
  }
...
/* link 颜色变体 */
.custom-button--link-primary {
  color: var(--primary-color);
}

.custom-button--link-primary:hover:not(:disabled) {
  color: var(--primary-dark);
}

.custom-button--link-success {
  color: var(--color-emerald);
}

.custom-button--link-success:hover:not(:disabled) {
  filter: brightness(0.9);
}

.custom-button--link-warning {
...
.custom-button--link-danger:hover:not(:disabled) {
